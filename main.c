#include "graphics.h"
#include "robot.h"
#include "maps.h"
#include "path_planning.h"
#include <stdlib.h>
#include <time.h>

#define GRID_SIZE 40
#define WINDOW_WIDTH (COLS * GRID_SIZE)
#define WINDOW_HEIGHT (ROWS * GRID_SIZE)
#define MARKER_COUNT 30
#define DELAY 20
#define TEST_COURSE 5  // Select test course (1-7)

// DRAWING FUNCTIONALITIES

void drawGridLines(){
    for (int r = 0; r <= ROWS; r++){
        setColour((r == 0 || r == ROWS) ? red : black);
        drawLine(0, r * GRID_SIZE, WINDOW_WIDTH, r * GRID_SIZE);
    }
    for (int c = 0; c <= COLS; c++){
        setColour((c == 0 || c == COLS) ? red : black);
        drawLine(c * GRID_SIZE, 0, c * GRID_SIZE, WINDOW_HEIGHT);
    }
}

void drawObstacles(int map[COLS][ROWS]){
    setColour(darkgray);
    for (int x = 0; x < COLS; x++){
        for (int y = 0; y < ROWS; y++){
            if (map[x][y] == -1){
                fillRect(x * GRID_SIZE, y * GRID_SIZE, GRID_SIZE, GRID_SIZE);
            }
        }
    }
}

void drawMarkers(Marker markers[]){
    setColour(gray);
    for (int i = 0; i < MARKER_COUNT; i++) {
        fillArc(markers[i].x * GRID_SIZE + GRID_SIZE / 4, markers[i].y * GRID_SIZE + GRID_SIZE / 4, GRID_SIZE / 2, GRID_SIZE / 2, 0, 360);
    }
}

void drawTriangle(int centerX, int centerY, int s, int direction){
    // Triangle coordinates for each direction
    const int triangles[4][3][2] = {
        // Direction 0 (up): tip at top
        {{centerX, centerY - s}, {centerX - s, centerY + s}, {centerX + s, centerY + s}},
        // Direction 1 (right): tip at right
        {{centerX + s, centerY}, {centerX - s, centerY - s}, {centerX - s, centerY + s}},
        // Direction 2 (down): tip at bottom
        {{centerX, centerY + s}, {centerX - s, centerY - s}, {centerX + s, centerY - s}},
        // Direction 3 (left): tip at left
        {{centerX - s, centerY}, {centerX + s, centerY - s}, {centerX + s, centerY + s}}
    };

    int xPoints[3] = {triangles[direction][0][0], triangles[direction][1][0], triangles[direction][2][0]};
    int yPoints[3] = {triangles[direction][0][1], triangles[direction][1][1], triangles[direction][2][1]};
    fillPolygon(3, xPoints, yPoints);
}

void drawRobot(Robot robot){
    setColour(blue);
    int centerX = robot.x * GRID_SIZE + GRID_SIZE / 2;
    int centerY = robot.y * GRID_SIZE + GRID_SIZE / 2;
    int s = GRID_SIZE / 3;
    drawTriangle(centerX, centerY, s, robot.direction);
}

void drawMovingObjects(Robot robot, Marker markers[]){
    clear(); // everytime there's a movement, update the elements that moved
    drawRobot(robot);
    drawMarkers(markers);
    sleep(DELAY);
}

// MAIN FUNCTION HELPERS

void setupSelectedTestCourse(int map[COLS][ROWS]) {
    switch (TEST_COURSE) {
        case 1: setupTestCourse1(map); break;
        case 2: setupTestCourse2(map); break;
        case 3: setupTestCourse3(map); break;
        case 4: setupTestCourse4(map); break;
        case 5: setupTestCourse5(map); break;
        case 6: setupTestCourse6(map); break;
        case 7: setupTestCourse7(map); break;
    }
}

void initializeRobot(Robot* robot, int map[COLS][ROWS]) {
    robot->x = rand() % COLS;
    robot->y = rand() % ROWS;
    while (map[robot->x][robot->y] == -1) {
        robot->x = rand() % COLS;
        robot->y = rand() % ROWS;
    }
}

void initializeMarkers(Marker markers[], Robot robot, int map[COLS][ROWS]) {
    for (int i = 0; i < MARKER_COUNT; i++) {
        int x = rand() % COLS;
        int y = rand() % ROWS;
        while ((map[x][y] != 0) || (x == robot.x && y == robot.y)) {
            x = rand() % COLS;
            y = rand() % ROWS;
        }
        markers[i].x = x;
        markers[i].y = y;
        markers[i].isCarried = 0;
        map[x][y] = i + 1;
    }
}

void initializeGraphics(int map[COLS][ROWS], Robot robot, Marker markers[]) {
    setWindowSize(WINDOW_WIDTH + 1, WINDOW_HEIGHT + 1);
    background();
    drawGridLines();
    drawObstacles(map);
    foreground();
    drawRobot(robot);
    drawMarkers(markers);
}

void runCoverageAlgorithm(Robot* robot, Marker markers[], int map[COLS][ROWS], float heuristic[COLS][ROWS]) {
    PlannerState state = COVERAGE_SEARCH;

    while (state != FOUND && state != NOT_FOUND) {
        if (state == COVERAGE_SEARCH) {
            if (coverageSearch(robot, markers, map, robot->knowledge, heuristic)) {
                state = FOUND;
            } else {
                state = NEAREST_UNVISITED_SEARCH;
            }
        }
        else if (state == NEAREST_UNVISITED_SEARCH) {
            createHeuristic(robot->x, robot->y, HEURISTIC_MANHATTAN, heuristic);
            if (aStarNavigate(robot, markers, map, robot->knowledge, heuristic, -1, -1, 1)) {
                state = COVERAGE_SEARCH;
                createHeuristic(robot->x, robot->y, HEURISTIC_VERTICAL, heuristic);
            } else {
                state = NOT_FOUND;
            }
        }
    }
}

void deliverRemainingMarkers(Robot* robot, Marker markers[], int map[COLS][ROWS]) {
    if (markerCount(*robot) == 0) return;

    int corner_x, corner_y;
    if (!findNearestCorner(*robot, &corner_x, &corner_y)) return;

    float corner_heuristic[COLS][ROWS];
    createHeuristic(corner_x, corner_y, HEURISTIC_MANHATTAN, corner_heuristic);

    if (aStarNavigate(robot, markers, map, robot->knowledge, corner_heuristic, corner_x, corner_y, 0)) {
        if (checkAtCorner(*robot)) {
            dropMarker(robot, markers, map);
        }
    }
}

int main(int argc, char const *argv[])
{
    srand(time(NULL));

    Marker markers[MARKER_COUNT];
    int map[COLS][ROWS] = {0};
    Robot robot = {0, 0, 0, 0, {{0}}};

    setupSelectedTestCourse(map);
    initializeRobot(&robot, map);
    initializeMarkers(markers, robot, map);

    float heuristic[COLS][ROWS];
    createHeuristic(robot.x, robot.y, HEURISTIC_VERTICAL, heuristic);

    initializeGraphics(map, robot, markers);
    runCoverageAlgorithm(&robot, markers, map, heuristic);
    deliverRemainingMarkers(&robot, markers, map);

    while (1) {
        sleep(1000);
    }
    return 0;
}